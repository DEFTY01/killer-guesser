...
| `PATCH` | `/api/admin/games/[id]` | `src/app/api/admin/games/[id]/route.ts` | Admin (403) | Update game state: action "update_vote_window" (set vote_window_start/end as HH:MM), "update_timezone" (set IANA timezone string), "close_voting" (null vote window + publish VOTE_CLOSED with results), "close" (set status=closed), "delete" (hard delete + cascade) |
...
| `GET` | `/api/game/[id]/vote` | `src/app/api/game/[id]/vote/route.ts` | Player (401) | Evening vote state — compares current time in game.timezone to `vote_window_start`/`vote_window_end`; open: returns alive players + aggregate tallies + `window_open_utc_ms`/`window_close_utc_ms`; closed: lazy-closes (majority elimination → VOTE_CLOSED), returns results; `see_votes` → full voter→target detail |
